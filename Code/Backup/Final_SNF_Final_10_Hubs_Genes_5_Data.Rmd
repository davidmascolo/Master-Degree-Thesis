...
df_10_hubs <- read.csv(paste0(path, "/Data/10%_hubs_diff_coex.txt"),
                       header = F, sep = ",")
df_10_hubs <- df_10_hubs$V1
...
```{r}
## Remove some parts of ID Patient for better visualization
rownames(df_rna_counts_final) <- sub("^SU2CLC-|T1$", "",
                                     rownames(df_rna_counts_final))
rownames(df_wes_final) <- sub("^SU2CLC-|T1$", "",
                                     rownames(df_wes_final))
rownames(df_immune_final) <- sub("^SU2CLC-|T1$", "",
                                     rownames(df_immune_final))
rownames(df_curated_final) <- sub("^SU2CLC-|T1$", "",
                                     rownames(df_curated_final))
rownames(df_myeloid_final) <- sub("^SU2CLC-|T1$", "",
                                     rownames(df_myeloid_final))
```
...
## Clustering with best C <- 2
final_clusters <- spectralClustering(W, K = C)

## Plot
optimal_cluster(W, C, true_label)
